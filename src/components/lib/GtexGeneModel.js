 ...
 *
 * Vendored from gtex-viz's src/modules/GeneModel.js, with d3-shape/d3-array/d3-scale
 * submodule imports collapsed onto this project's top-level `d3` package (same API,
 * same v7 major as the original's v5 -- no behavior changes from the original).
 */
/*
...
export default class GtexGeneModel {
    /**
     * constructor
    ...
    setXscaleFixIntron(w){
        ...
        const maxExonLength = d3.max(this.exons, (d)=>d.length);
        ...
        const totalExonLength = d3.sum(this.exons, (d)=>d.length>minLength?d.length:minLength); // if an exon is shorter than min length, use min length
